 ...
 * `ios/BakersLiveActivityModule.swift` à `app/production/[id].tsx`. Chaque
...
declare class BakersLiveActivityNativeModule extends NativeModule<Record<string, never>> {
  start(recipeTitle: string, stepText: string, endAtIso: string | null): void;
  update(stepText: string, endAtIso: string | null): void;
  end(): void;
}

let native: BakersLiveActivityNativeModule | null | undefined;
function getNative(): BakersLiveActivityNativeModule | null {
  if (Platform.OS !== 'ios') return null;
  if (native === undefined) {
    try {
      native = requireNativeModule<BakersLiveActivityNativeModule>('BakersLiveActivity');
    } catch {
      native = null;
    }
  }
